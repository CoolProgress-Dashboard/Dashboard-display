/**
 * Populate Supabase peak_load_data table
 *
 * Usage:
 *   SUPABASE_SERVICE_KEY=eyJ... node scripts/populate_peak_load_data.mjs
 *
 * Prerequisites:
 *   - Table created via: sql/create_peak_load_data.sql
...
async function main() {
  console.log('Populating peak_load_data...');
  const count = await insertBatch('peak_load_data', ROWS);
  console.log(`Done: ${count} rows inserted into peak_load_data`);
}
