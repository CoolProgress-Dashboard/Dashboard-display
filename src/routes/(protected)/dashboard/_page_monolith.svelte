<script lang="ts">
  ...
  let mepsDataProp: Meps[] = [];
  let countriesProp: Country[] = [];

  // Top-level reactive props for AccessPillar (updated from onMount after data load)
  let accessDataProp: import('$lib/services/dashboard-types').AccessRecord[] = [];
  let accessForecastProp: import('$lib/services/dashboard-types').AccessForecastRecord[] = [];

  // Top-level reactive props for PolicyPillar (updated from onMount after data load)
  let policyPledgeProp: import('$lib/services/dashboard-types').Pledge[] = [];
  let policyNdcTrackerProp: import('$lib/services/dashboard-types').NdcTrackerRecord[] = [];
  let policyNcapProp: import('$lib/services/dashboard-types').NcapRecord[] = [];
  let policyCountriesProp: import('$lib/services/dashboard-types').Country[] = [];
  let policyKigaliProp: import('$lib/services/dashboard-types').Kigali[] = [];
  let policyMepsProp: Meps[] = [];
      ...
      if (currentView === 'emissions') {
        ...
        function switchView(view: string | undefined) {
            ...
            const reinitViewCharts = () => {
                ...
                if (view === 'access') {
                    // Delegate to the self-contained AccessPillar component
                    if (typeof (window as any).__accessRenderTimeline === 'function') {
                        (window as any).__accessRenderTimeline();
                    } else {
                        renderAccessTimeline(); // fallback to legacy closure
                    }
                }
                if (view === 'policy') {
        ...
        // Callbacks for AccessPillar to sync country selection back to the page-level state
        (window as any).__dashboardSetCountry = (code: string) => {
            selectedCountry = code;
            globalCountryFilter = code;
            const countryFilter = document.getElementById('country-filter') as HTMLSelectElement | null;
            if (countryFilter) countryFilter.value = code;
            highlightCountryOnMaps(code);
            updateViewingBadges();
            updateViewsForCountryFilter();
        };
        ...
        async function init() {
            try {
                ...
                mepsDataProp = data.meps; // sync raw MEPS records for MepsPillar D3 map
                countriesProp = data.countries; // sync countries list for MepsPillar D3 map
                accessDataProp = data.access; // sync access records for AccessPillar
                accessForecastProp = data.accessForecast; // sync forecast records for AccessPillar
                // sync data props for PolicyPillar
                policyPledgeProp = data.pledge;
                policyNdcTrackerProp = data.ndcTracker;
                policyNcapProp = data.ncap;
                policyCountriesProp = data.countries;
                policyKigaliProp = data.kigali;
                policyMepsProp = data.meps;
  ...
  <div class="main-container" class:overview-only={currentViewState === 'overview'}>
    ...
    <main class="main-content">
      ...
      <MepsPillar
        ...
        mepsData={mepsDataProp}
        countries={countriesProp}
      />
      <KigaliPillar active={currentViewState === 'kigali'} onPillarInfoClick={handlePillarInfoClick} />
      <AccessPillar
        active={currentViewState === 'access'}
        onPillarInfoClick={handlePillarInfoClick}
        accessData={accessDataProp}
        accessForecast={accessForecastProp}
        countries={countriesProp}
      />
      <PolicyPillar
        active={currentViewState === 'policy'}
        onPillarInfoClick={handlePillarInfoClick}
        pledge={policyPledgeProp}
        ndcTracker={policyNdcTrackerProp}
        ncap={policyNcapProp}
        countries={policyCountriesProp}
        kigali={policyKigaliProp}
        meps={policyMepsProp}
      />
